...
def find_files(dirname, match=None):
    return [x.split("/")[-1] for x in glob.glob(f"{dirname}/*{match}*")]
...
class MorphRater(QMainWindow, mainwindow.Ui_MainWindow):
    ...
    colour_codes = [QColor(200, 200, 200), QColor(27, 76, 198), QColor(234, 224, 23), QColor(198, 13, 13), QColor(30, 180, 30), Qt.gray]
    categories = ["Irregular", "Elliptical", "Disk", "Disk+Bulge", "Discard"]

    def __init__(self, image_dir):
        super(MorphRater, self).__init__()
        self.setupUi(self)
        ...
        self.wtf = defaultdict(bool)
        self.setChildrenFocusPolicy(QtCore.Qt.NoFocus)
        QtWidgets.qApp.installEventFilter(self)

        self.radios = [self.radio_0, self.radio_1, self.radio_2, self.radio_3, self.radio_4]
        ...
        self.button_wtf.clicked.connect(self.set_wtf)
        ...
        self.edit_comment.editingFinished.connect(self.comment_changed)
    ...
    def set_wtf(self):
        self.wtf[self.current_image] = not self.wtf[self.current_image]
        comment = self.comments[self.current_image]
        if comment.startswith("WTF! "):
            comment = comment[4:]
        if self.wtf[self.current_image]:
            comment = "WTF! " + comment
        self.comments[self.current_image] = comment
        self.edit_comment.setText(comment)
    ...
    def comment_changed(self):
        self.comments[self.current_image] = self.edit_comment.text()
        self.setFocus()
    ...
    def setChildrenFocusPolicy(self, policy):
        def recursiveSetChildFocusPolicy(parentQWidget):
            for childQWidget in parentQWidget.findChildren(QWidget):
                if childQWidget == self.username_edit or childQWidget == self.jump_box or childQWidget == self.edit_comment:
                    continue
    ...
    def eventFilter(self, source, event):
        if event.type() == QtCore.QEvent.KeyPress:
            if event.key() == QtCore.Qt.Key_Left or event.key(
                ...
                return True
        return super(MorphRater, self).eventFilter(source, event)
    ...
    def goto_image(self, index):
        ...
        current_comment = self.scores[self.current_image]
        self.edit_comment.setText(self.comments[self.current_image])
        self.set_colour_code(MorphRater.colour_codes[current_score])
    ...
    def score_image(self, score):
        ...
        self.set_colour_code(MorphRater.colour_codes[score])
        self.toggle_radio(score)
        self.colour_label.setText(MorphRater.categories[score] + 
                (" (!)" if self.wtf[self.current_image] else ""))
    ...
    def load(self):
        if os.path.isfile(self.scorefile):
            ...
            for line in lines[1:]:
                ...
                self.comments[line[1]] = line[3]
                self.wtf[line[1]] = line[3].startswith("WTF!")
    ...
    def save(self):
        with open(self.scorefile, "w") as f:
            f.write("username,image,score,comment\n")
            for file_ in self.image_files:
                f.write("%s,%s,%d,%s\n" % (self.username, file_.split("/")[-1],\
                        self.scores[file_], ("WTF! " if self.wtf[file_] else "" + self.comments[file_]).strip()))
